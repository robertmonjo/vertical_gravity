...
Weights:
  ...
  nbar>=2 →  w_i(R) ∝ 1/chi2_local_i(R) via log(R) kernel, sigma_log=0.50
...
def compute_kernel_weights(reconstructions, r_band, centers, r_obs, v_obs, sigma_obs,
    ...
    for i, logR in enumerate(log_r_band):
        if sigma_log == float("inf") or sigma_log <= 0:
            K = np.ones(len(r_obs))
        else:
...
def main():
    ...
    parser.add_argument(
        "--sigma-log", type=float, default=SIGMA_LOG_DEFAULT, metavar="SIGMA",
        help=f"Log-radius kernel width for local likelihood weighting (default {SIGMA_LOG_DEFAULT}). "
             "Use 0 or inf for flat global weights (no locality bias).",
    )
